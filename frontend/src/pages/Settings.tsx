...
export default function Ayarlar() {
    ...
    const sifreDegistir = async (e: React.FormEvent) => {
        ...
        if (yeniSifre !== sifreTekrar) {
            setSifreMesaji({ tip: 'hata', metin: 'Yeni şifreler eşleşmiyor' });
            return;
        }

        if (yeniSifre.length < 8) {
            setSifreMesaji({ tip: 'hata', metin: 'Şifre en az 8 karakter olmalı' });
            return;
        }

        try {
            const yanit = await axios.post('/api/kimlik/sifre-degistir', {
    ...
    const sistemIslemi = async (islem: 'yenidenbaslat' | 'kapat') => {
        const mesaj = islem === 'yenidenbaslat'
            ? 'Sistem yeniden başlatılacak. Emin misiniz?'
            : 'Sistem kapatılacak. Emin misiniz?';

        if (confirm(mesaj)) {
            try {
                await axios.post(`/api/sistem/${islem}`);
                alert(`${islem === 'yenidenbaslat' ? 'Yeniden başlatma' : 'Kapatma'} komutu gönderildi`);
            } catch (hata) {
                console.error('Sistem işlem hatası:', hata);
            }
        }
    };
                ...
                <div className="lg:w-64 glass-card rounded-2xl p-4 space-y-2">
                    {sekmeler.map((sekme) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${aktifSekme === sekme.id
                                    ? 'bg-primary-500/20 text-primary-400 border border-primary-500/30'
                                    : 'text-gray-400 hover:text-white hover:bg-white/5'
                                }`}
                        >
                            <sekme.ikon className="w-5 h-5" />
                            <span className="font-medium">{sekme.etiket}</span>
                        </button>
                    ))}
                </div>
                ...
                <div className="flex-1 glass-card rounded-2xl p-6">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid gap-4">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={kullanici?.kullaniciAdi || 'admin'}
                                        disabled
                    ...
                    {aktifSekme === 'guvenlik' && (
                        <div className="space-y-6">
                            ...
                            {sifreMesaji && (
                                <div className={`px-4 py-3 rounded-xl text-sm ${sifreMesaji.tip === 'basari'
                            ...
                            <form onSubmit={sifreDegistir} className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        type={sifreleriGoster ? 'text' : 'password'}
                                        value={mevcutSifre}
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        type="button"
                                        onClick={() => setSifreleriGoster(!sifreleriGoster)}
                                        className="text-gray-400 hover:text-white text-sm flex items-center gap-1"
                                    >
                                        {sifreleriGoster ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                        {sifreleriGoster ? 'Gizle' : 'Göster'}
                                    </button>
                                </div>
                                <button
                                    type="submit"
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid gap-4">
                                <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                                    ...
                                    <button
                                        onClick={() => sistemIslemi('yenidenbaslat')}
