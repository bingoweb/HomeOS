    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
                ...
                <button onClick={konteynerleriGetir} className="flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-xl transition-colors">
            ...
            <div className="relative">
                ...
                <input type="text" value={arama} onChange={(e) => setArama(e.target.value)} placeholder="Konteyner ara..." className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500/50" />
            </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    {konteyner.calismaDurumu === 'running' ? (
                                        <button onClick={() => islemYap(konteyner.id, 'durdur')} disabled={islemYukleniyor === konteyner.id} className="p-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-xl transition-colors" title="Durdur">
                                            <Square className="w-5 h-5" />
                                        </button>
                                    ) : (
                                        <button onClick={() => islemYap(konteyner.id, 'baslat')} disabled={islemYukleniyor === konteyner.id} className="p-2 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-xl transition-colors" title="Başlat">
                                            <Play className="w-5 h-5" />
                                        </button>
                                    )}
                                    <button onClick={() => islemYap(konteyner.id, 'yenidenbaslat')} disabled={islemYukleniyor === konteyner.id} className="p-2 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-400 rounded-xl transition-colors" title="Yeniden Başlat">
                                        <RefreshCw className="w-5 h-5" />
                                    </button>
                                    <button onClick={() => loglariGoster(konteyner.id)} className="p-2 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-colors" title="Loglar">
                                        <Terminal className="w-5 h-5" />
                                    </button>
                                    <button onClick={() => islemYap(konteyner.id, 'sil')} disabled={islemYukleniyor === konteyner.id} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl transition-colors" title="Sil">
